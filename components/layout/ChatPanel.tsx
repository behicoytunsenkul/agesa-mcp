...
export function ChatPanel() {
  ...
  const { chatOpen, setChatOpen, toggleChat } = useLayout();
  ...
  return (
    <>
      ...
      {/* Desktop spacer — reserves width so main content resizes smoothly */}
      <div
        aria-hidden
        className={`hidden shrink-0 transition-[width] duration-300 ease-out lg:block ${
          chatOpen ? "w-[min(380px,36vw)] xl:w-[380px]" : "w-0"
        }`}
      />

      <aside
        className={`fixed inset-y-0 right-0 z-50 flex w-[min(100vw,400px)] flex-col border-l border-[var(--border)] bg-[var(--surface)] shadow-2xl transition-transform duration-300 ease-out sm:w-[min(92vw,380px)] lg:w-[min(380px,36vw)] xl:w-[380px] lg:shadow-none ${
